...
<br>
<br>
💡 C/C++ 기반으로 Linux 커널 및 하드웨어 저수준 계층과 상호작용하는 임베디드 시스템을 개발합니다. 디바이스 드라이버 구현과 I2C·GPIO 기반 하드웨어 제어를 중심으로, 부트로더부터 커널 영역까지의 동작 흐름을 실제 시스템 관점에서 이해하고 구현해왔습니다. 제한된 하드웨어 리소스에서 안정성과 신뢰성을 갖춘 시스템을 설계하는 것을 지향합니다.
...
<br>
<br>
💡 C/C++ベースで Linux カーネルおよびハードウェアの低レイヤ層と相互作用する組み込みシステムを開発しています。デバイスドライバの実装と I2C・GPIO ベースのハードウェア制御を中心に、ブートローダーからカーネル領域までの動作フローを実際のシステム観点から理解し、実装してきました。限られたハードウェアリソースの中で、安定性と信頼性を備えたシステムを設計することを志向しています。
